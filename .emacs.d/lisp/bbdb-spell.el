...
;; Mark Simpson for letting me know it didn't work with 2.35
...
(defvar bbdb-spell-old (fboundp 'bbdb-record-get-field-internal))

(if bbdb-spell-old
    (defalias 'bbdb-spell-get-field 'bbdb-record-get-field-internal)
  (defalias 'bbdb-spell-get-field 'bbdb-record-get-field))
  ...
  (save-window-excursion
    (dolist (record (bbdb-search (bbdb-records) ""))
      (dolist (field bbdb-spell-field)
        (bbdb-spell-export-field
         (if (and bbdb-spell-old
                  (eq field 'organization))
             (bbdb-record-company record)
           (bbdb-spell-get-field record field))))))
  (mapc (lambda (lang)
